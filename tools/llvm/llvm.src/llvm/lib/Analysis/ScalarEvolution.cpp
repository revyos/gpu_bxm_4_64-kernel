      ...
      if (ConstantInt *CI = dyn_cast<ConstantInt>(BO->RHS)) {
        ...
        if ((LZ != 0 || TZ != 0) && !((~A & ~Known.Zero) & EffectiveMask)) {
          ...
          if (!ShiftedLHS)
           ...
           Type* newIntTy = cast<IntegerType>(getDataLayout().getSmallestLegalIntType(getContext(), BitWidth - LZ - TZ));
           if (BitWidth - LZ - TZ == 1)
             newIntTy = IntegerType::get(getContext(), 1);
           if (newIntTy->getScalarSizeInBits() != BitWidth - LZ - TZ)
		   // we didn't simplify. we need to return an error.
                   return getUnknown(V);
           return getMulExpr(
               getZeroExtendExpr(
