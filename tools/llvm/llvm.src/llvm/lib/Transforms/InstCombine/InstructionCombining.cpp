...
static void propagateMediumPrecisionMDIfUniform(const Value* src1, const Value* src2, Value* dst) {
  unsigned mediumpPrecisionKind = src1->getContext().getMDKindID("mediumPrecision");
  const Instruction* srcInst1 = dyn_cast<Instruction>(src1);
  const Instruction* srcInst2 = dyn_cast<Instruction>(src2);
  Instruction* dstInst = dyn_cast<Instruction>(dst);
  if (!srcInst1 || !srcInst1 || !dstInst) return;
  MDNode* mediumpPrecisionMD1 = srcInst1->getMetadata(mediumpPrecisionKind);
  MDNode* mediumpPrecisionMD2 = srcInst2->getMetadata(mediumpPrecisionKind);
  if (mediumpPrecisionMD1 && mediumpPrecisionMD2)
    dstInst->setMetadata(mediumpPrecisionKind, mediumpPrecisionMD1);
}
...
bool InstCombiner::shouldChangeType(unsigned FromWidth,
  ...
  // Upcasting to 64 bits is expensive for RGX as only some ops are native (ADD64)
  if (ToWidth > 32)
    return false;
      ...
      if (Op0 && Op1 &&
        ...
        bool IsNUW = hasNoUnsignedWrap(I) &&
         ...
         if (isa<FPMathOperator>(NewBO)) {
          ...
          propagateMediumPrecisionMDIfUniform(Op0, Op1, NewBO);
        }
...
Instruction *InstCombiner::visitSwitchInst(SwitchInst &SI) {
  ...
  if (NewWidth > 0 && NewWidth < Known.getBitWidth() && DL.isLegalInteger(NewWidth)) {
