...
Instruction *InstCombiner::foldICmpMulConstant(ICmpInst &Cmp,
  ...
  if ((Mul->hasNoSignedWrap() &&
    ...
    if (ReduceSigned || ReduceUnsigned || ReduceEquality) {
      ...
      } else if ((ReduceUnsigned && C.ugt(*MulC)) ||
                 (ReduceSigned && C.sgt(*MulC) && MulC->sgt(0))) {
        // If it does not divide evenly the comparison can still be reduced
        // but ReducedVal needs to be incremented for SLT and ULT.
        // For example: x * 4 < 21 --> x < (5 + 1)
        // while x * 4 > 21 --> x > 5
        if (Pred == ICmpInst::ICMP_SLT || Pred == ICmpInst::ICMP_ULT) {
          return new ICmpInst(Pred, Mul->getOperand(0),
                              ConstantInt::get(OpType, ReducedVal + 1));
        } else if (Pred == ICmpInst::ICMP_SGT || Pred == ICmpInst::ICMP_UGT) {
          return new ICmpInst(Pred, Mul->getOperand(0),
                              ConstantInt::get(OpType, ReducedVal));
        }
      } else if (ReduceEquality) {
        // When the comparison is equality, and remainder is non zero, it will
        // always be true if NE or false in EQ.
        // For example: x * 3 == 20 can never be true.
        Value *RetVal =
            Pred == ICmpInst::ICMP_EQ ? Builder.getFalse() : Builder.getTrue();
        return replaceInstUsesWith(Cmp, RetVal);
      }
    }
  }
...
Instruction *InstCombiner::visitFCmpInst(FCmpInst &I) {
  ...
  if (match(Op0, m_FNeg(m_Value(X)))) {
    ...
    if (match(Op1, m_Constant(C))) {
      ...
      FCmpInst* New = new FCmpInst(I.getSwappedPredicate(), X, NegC, "", &I);
      New->setFastMathFlags(I.getFastMathFlags());
      return New;
    }
  }
