...
static void setMediumPrecisionFlag(const Value* src, Value* dst) {
  if (!src || !isa<Instruction>(src)) return;
  if (!dst || !isa<Instruction>(dst)) return;
  Instruction* destInst = cast<Instruction>(dst);
  const Instruction* srcInst = cast<const Instruction>(src);
  unsigned mediumpPrecisionKind = srcInst->getContext().getMDKindID("mediumPrecision");
  MDNode* mediumpPrecisionMD1 = srcInst->getMetadata(mediumpPrecisionKind);
  if (mediumpPrecisionMD1)
    destInst->setMetadata(mediumpPrecisionKind, mediumpPrecisionMD1);
}

static void CopyFMF(BinaryOperator* Res, Value* FlagsOp)
{
  if (auto* I = dyn_cast<Instruction>(FlagsOp)) {
    Res->copyFastMathFlags(I);
  } else {
    Res->setFastMathFlags(cast<FPMathOperator>(FlagsOp)->getFastMathFlags());
  }
  setMediumPrecisionFlag(FlagsOp, Res);
}
  ...
  for (unsigned i = 0; ; ++i) {
    ...
    if (NodesToRewrite.empty()) {
      ...
      if (NewOp->getType()->isFPOrFPVectorTy()) {
        ...
        setMediumPrecisionFlag(I, NewOp);
      }
    } else {
...
Instruction *ReassociatePass::canonicalizeNegConstExpr(Instruction *I) {
  ...
  case Instruction::FAdd:
    ...
    setMediumPrecisionFlag(User, NI);
    break;
