...
public:
  SeparateConstOffsetFromGEP(const DataLayout *DL, DominatorTree &DT,
                             ScalarEvolution &SE, LoopInfo &LI,
                             TargetLibraryInfo &TLI,
                             std::function<TargetTransformInfo &()> LookupTargetTransformInfo_,
                             bool LowerGEP)
                             : DL(DL), DT(&DT), SE(&SE), LI(&LI), TLI(&TLI),
                               LookupTargetTransformInfo(LookupTargetTransformInfo_),
                               LowerGEP(LowerGEP) {}
...
private:
  ...
  std::function<TargetTransformInfo &()> LookupTargetTransformInfo;
...
bool SeparateConstOffsetFromGEP::canonicalizeArrayIndicesToPointerSize(
  ...
  bool Changed = false;
  Type *IntIdxTy = DL->getIndexType(GEP->getPointerOperandType());
  gep_type_iterator GTI = gep_type_begin(*GEP);
  for (User::op_iterator I = GEP->op_begin() + 1, E = GEP->op_end();
       I != E; ++I, ++GTI) {
    // Skip struct member indices which must be i32.
    if (GTI.isSequential()) {
      if ((*I)->getType() != IntIdxTy) {
        *I = CastInst::CreateIntegerCast(*I, IntIdxTy, true, "idxprom", GEP);
        Changed = true;
      }
    }
  }
...
void SeparateConstOffsetFromGEP::lowerToSingleIndexGEPs(
  ...
  Type *IntIdxTy = DL->getIndexType(Variadic->getPointerOperandType());
  ...
  for (unsigned I = 1, E = Variadic->getNumOperands(); I != E; ++I, ++GTI) {
    if (GTI.isSequential()) {
      ...
      APInt ElementSize = APInt(IntIdxTy->getIntegerBitWidth(),
                                DL->getTypeAllocSize(GTI.getIndexedType()));
      // Scale the index by element size.
      if (ElementSize != 1) {
        if (ElementSize.isPowerOf2()) {
          Idx = Builder.CreateShl(
              Idx, ConstantInt::get(IntIdxTy, ElementSize.logBase2()));
        } else {
          Idx = Builder.CreateMul(Idx, ConstantInt::get(IntIdxTy, ElementSize));
        }
      }
  ...
  if (AccumulativeByteOffset != 0) {
    Value *Offset = ConstantInt::get(IntIdxTy, AccumulativeByteOffset);
    ResultPtr =
...
SeparateConstOffsetFromGEP::lowerToArithmetics(GetElementPtrInst *Variadic,
  ...
  Value *ResultPtr = Builder.CreatePtrToInt(Variadic->getOperand(0), IntIdxTy);
  gep_type_iterator GTI = gep_type_begin(*Variadic);
  ...
  if (AccumulativeByteOffset != 0) {
    ResultPtr = Builder.CreateAdd(
        ResultPtr, ConstantInt::get(IntIdxTy, AccumulativeByteOffset));
  }
...
bool SeparateConstOffsetFromGEP::splitGEP(GetElementPtrInst *GEP) {
  ...
  TargetTransformInfo &TTI = LookupTargetTransformInfo();
  ...
  if (AccumulativeByteOffset % ElementTypeSizeOfGEP == 0) {
    ...
    NewGEP = GetElementPtrInst::Create(GEP->getResultElementType(), NewGEP,
                                       ConstantInt::get(IntIdxTy, Index, true),
                                       GEP->getName(), GEP);
    ...
    NewGEP = GetElementPtrInst::Create(
        ...
        ConstantInt::get(IntIdxTy, AccumulativeByteOffset, true), "uglygep",
        GEP);
    NewGEP->copyMetadata(*GEP);
    // Inherit the inbounds attribute of the original GEP.
    cast<GetElementPtrInst>(NewGEP)->setIsInBounds(GEPWasInBounds);
...
public:
  ...
  SeparateConstOffsetFromGEPLegacyPass(bool LowerGEP = false)
      : FunctionPass(ID), LowerGEP(LowerGEP) {
	    initializeSeparateConstOffsetFromGEPLegacyPassPass(*PassRegistry::getPassRegistry());
  	  }
  ...
  bool runOnFunction(Function &F) override {
	...
	auto LookupTTI = [this, &F]() -> TargetTransformInfo & {
	  return getAnalysis<TargetTransformInfoWrapperPass>().getTTI(F);
	};

	SeparateConstOffsetFromGEP SCOFG(DL, DT, SE, LI, TLI, LookupTTI, LowerGEP);
	return SCOFG.run(F);
  }
};
...
PreservedAnalyses SeparateConstOffsetFromGEPPass::run(Function &F,
  ...
  auto LookupTTI= [&]() -> TargetTransformInfo & {
