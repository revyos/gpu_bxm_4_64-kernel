...
void Instruction::copyFastMathFlags(const Instruction *I) {
  ...
  unsigned mediumpPrecisionKind = getContext().getMDKindID("mediumPrecision");
  MDNode* mediumpPrecisionMD = I->getMetadata(mediumpPrecisionKind);
  if (mediumpPrecisionMD != nullptr) {
    setMetadata(mediumpPrecisionKind, mediumpPrecisionMD);
  }
}

void Instruction::andMediumPrecisionFlag(const Instruction *I) {
  unsigned mediumpPrecisionKind = getContext().getMDKindID("mediumPrecision");
  MDNode* mediumpPrecisionMD = getMetadata(mediumpPrecisionKind);
  if (I->getMetadata(mediumpPrecisionKind) == nullptr) {
    mediumpPrecisionMD = nullptr;
  }
  setMetadata(mediumpPrecisionKind, mediumpPrecisionMD);
}
...
void Instruction::andIRFlags(const Value *V) {
  ...
  if (auto *FP = dyn_cast<FPMathOperator>(V)) {
    if (isa<FPMathOperator>(this)) {
      ...
      if (isa<Instruction>(this) && isa<Instruction>(V)) {
        cast<Instruction>(this)->andMediumPrecisionFlag(cast<Instruction>(V));
      }
    }
  }
