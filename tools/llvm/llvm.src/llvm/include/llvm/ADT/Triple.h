...
public:
  ...
  ArchType getArch() const {
    if (Arch == usc) return usc;
    if (Arch == usc64) return usc64;
    if (Arch == renderscript32) return renderscript32;
    if (Arch == renderscript64) return renderscript64;
    if (Arch == spir) return spir;
    if (Arch == spir64) return spir64;
    if (Arch == arm) return arm;
    if (Arch == aarch64) return aarch64;

    // Provide a default to avoid uninitialised issues because of default target triple
    // which generally is the system one.
    return usc64;
  }
  ...
  VendorType getVendor() const { return UnknownVendor; }

  /// getOS - Get the parsed operating system type of this triple.
  OSType getOS() const { return UnknownOS; }
