...
private:
  ...
  void setMediumPrecisionFlag(const Instruction* src, Instruction* dst) const {
    unsigned mediumpPrecisionKind = src->getContext().getMDKindID("mediumPrecision");
	 if (!src) return;
    MDNode* mediumpPrecisionMD1 = src->getMetadata(mediumpPrecisionKind);
    if (mediumpPrecisionMD1)
      dst->setMetadata(mediumpPrecisionKind, mediumpPrecisionMD1);
  }
...
public:
  ...
  Value *CreateFAddFMF(Value *L, Value *R, Instruction *FMFSource,
    ...
    setMediumPrecisionFlag(FMFSource, I);
    return Insert(I, Name);
  }
