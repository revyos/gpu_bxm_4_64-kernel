...
static void InsertOCLBuiltinDeclarationsFromTable(Sema &S, LookupResult &LR,
                                                  ...
                                                  const unsigned Len) {
  if (S.doneDecls.find(II->getName()) != S.doneDecls.end())
  {
    auto& declList = S.doneDecls[II->getName()];
    for (unsigned i = 0; i < declList.size(); ++i)
    {
      FunctionDecl *New = declList[i];
      LR.addDecl(New);
    }
    LR.resolveKind();
    return;
  ...
  for (unsigned SignatureIndex = 0; SignatureIndex < Len; SignatureIndex++) {
    ...
    for (unsigned Index = 0; Index < GenTypeMaxCnt; Index++) {
      ...
      S.doneDecls[II->getName()].push_back(NewOpenCLBuiltin);
